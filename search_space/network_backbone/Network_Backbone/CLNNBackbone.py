...
class CLNNBackbone(NetworkBackboneComponent):
    def __init__(self, **kwargs: Any):
        super().__init__(**kwargs)

    def build_backbone(self, input_shape: Tuple[int, ...]):
        return CLNN(self.config)
    
    @staticmethod
    def get_properties(dataset_properties: Optional[Dict[str, BaseDatasetPropertiesType]] = None) -> Dict[str, Any]:
        return {
            'shortname': 'CLNNBackbone',
            'name': 'CLNNBackbone',
...
class UnitB(nn.Module):
    def __init__(self, num_channels: int, kernel_size: int):
        ...
        self.branch1 = nn.Sequential(
            # standard conv
            nn.Conv1d(branch_channel, num_channels, kernel_size, padding="same",),
            nn.BatchNorm1d(num_channels),
            nn.SELU(inplace=True),

            nn.Conv1d(num_channels, branch_channel, kernel_size, padding="same",),
    ...
    def forward(self, x: torch.FloatTensor) -> torch.FloatTensor:
        x1 = x[:, :(x.shape[1]//2), :]
        x2 = x[:, (x.shape[1]//2):, :]
        out = torch.cat((x1, self.branch1(x2)), 1)
