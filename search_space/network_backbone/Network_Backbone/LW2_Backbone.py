...
class DCNNBackbone(NetworkBackboneComponent):
    def __init__(self, **kwargs: Any):
        super().__init__(**kwargs)

    def build_backbone(self, input_shape: Tuple[int, ...]):
        return DCNN(self.config)
    
    @staticmethod
    def get_properties(dataset_properties: Optional[Dict[str, BaseDatasetPropertiesType]] = None) -> Dict[str, Any]:
        return {
            'shortname': 'DCNNBackbone',
            'name': 'DCNNBackbone',
    ...
    def get_hyperparameter_search_space(
        ...
        num_sconv: HyperparameterSearchSpace = HyperparameterSearchSpace(
                                                    hyperparameter='num_sconv',
                                                    value_range=(1,4),
                                                    default_value=2,
                                                    ),                                              
        ...
        add_hyperparameter(cs, num_sconv, UniformIntegerHyperparameter)
...
class DCNN(nn.Module):
    def __init__(self, config):
        ...
        self.sconv1 = self._sconv(self.config["num_kernels"], self.num_grown_kernels)
        self.SE3 = SELayer(self.num_grown_kernels, self.config["reduction_ratio"])

        sconvi = list()
        for i in range(1, self.config["num_sconv"]):
    ...
    def _sconv(self, in_channel: int, out_channel: int) -> nn.Sequential:
        return nn.Sequential(
            # Depth-wise conv
            nn.Conv1d(in_channel, in_channel, self.config["kernel_size"], 
                      padding="same", groups=in_channel, bias=False),
